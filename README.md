...
- **PWA Web Notifications**: Integrated with the standard Web Notification API and a resource-conscious dynamic scheduler (`setTimeout`/`setInterval` adapting to target distance) for return reminders inside active browser tabs.
- **Client-Side Image Compression**: Utilizes Canvas rendering to downscale and compress item photos (max 500x500 pixels at 70% quality) to stay safely within `localStorage`'s 5MB size limit.
...
1. **Interactive Today Packing (2-Phase Flow)**:
   ...
   - **Quick Templates**: Apply template presets (e.g. "School", "Work", "Gym") to select all items in a group in one tap, keeping required items pre-checked.
...
5. **Streak Counter**:
   - Counts consecutive days with a perfect 100% packing record.
   - Displays a warm glowing/pulsing fire badge on active streaks and a motivational card in the history log.
...
- **Thông báo PWA Web**: Tích hợp với Web Notification API và bộ lập lịch động thông minh (tự động giãn thời gian kiểm tra dựa trên khoảng cách tới giờ hẹn) để nhắc nhở người dùng ra về trên trình duyệt.
- **Tự động nén ảnh**: Sử dụng Canvas để tự động thu nhỏ và nén chất lượng ảnh (tối đa 500x500px, chất lượng 70%) nhằm hoạt động mượt mà trong giới hạn 5MB của `localStorage`.
...
1. **Kiểm đồ hôm nay (Quy trình 2 pha)**:
   ...
   - **Chọn nhanh mẫu (Quick Templates)**: Áp dụng các mẫu có sẵn (vd: "Đi học", "Đi làm", "Đi gym") để chọn nhanh toàn bộ đồ vật trong nhóm đó chỉ với 1 lượt chạm, đồng thời giữ nguyên các đồ dùng bắt buộc.
...
5. **Đếm chuỗi ngày tích cực (Streak Counter)**:
   - Đếm số ngày liên tiếp check đủ 100% đồ mang về để tạo động lực xếp đồ đều đặn.
   - Huy hiệu ngọn lửa bùng cháy rực rỡ trên thanh Header và hiển thị thẻ động lực đáng yêu trong mục Lịch sử.
